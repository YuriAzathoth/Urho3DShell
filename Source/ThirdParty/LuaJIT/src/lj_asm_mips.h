...
static void asm_callx(ASMState* as, IRIns* ir)
{
	...
	{ /* Need specific register for indirect calls. */
...
static void asm_callround(ASMState* as, IRIns* ir, IRCallID id)
{
	...
	RegSet drop = RID2RSET(RID_R1) | RID2RSET(RID_R12) | RID2RSET(RID_FPRET) | RID2RSET(RID_F2) | RID2RSET(RID_F4) |
...
static void asm_retf(ASMState* as, IRIns* ir)
{
	...
	asm_guard(as, MIPSI_BNE, RID_TMP, ra_allock(as, i32ptr(pc), rset_exclude(RSET_GPR, base)));
...
static void asm_conv(ASMState* as, IRIns* ir)
{
	...
	if (irt_isfp(ir->t))
	{
		...
		{ /* U32 to FP conversion. */
			...
			emit_fgh(as, irt_isfloat(ir->t) ? MIPSI_ADD_S : MIPSI_ADD_D, dest, dest, tmp);
...
static void asm_strto(ASMState* as, IRIns* ir)
{
	...
	emit_tsi(as, MIPSI_ADDIU, ra_releasetmp(as, ASMREF_TMP1), RID_SP, ofs);
}
...
static void asm_uref(ASMState* as, IRIns* ir)
{
	...
	else
	{
		...
		emit_tsi(as, MIPSI_LW, uv, func, (int32_t)offsetof(GCfuncL, uvptr) + 4 * (int32_t)(ir->op2 >> 8));
	}
}
	...
	for (p++; p < pe; p++)
	{
		...
		{ /* Look for load of exit number. */
			...
			{ /* Look for exitstub branch. */
				...
				{ /* Branch out of range. Use spare jump slot in mcarea. */
					int i;
