...
static int asm_fusemadd(ASMState* as, IRIns* ir, PPCIns pi, PPCIns pir)
{
	...
	if (lref != rref &&
		((mayfuse(as, lref) && (irm = IR(lref), irm->o == IR_MUL) && ra_noreg(irm->r)) ||
		 (mayfuse(as, rref) && (irm = IR(rref), irm->o == IR_MUL) && (rref = lref, pi = pir, ra_noreg(irm->r)))))
	{
...
static void asm_tointg(ASMState* as, IRIns* ir, Reg left)
{
	...
	emit_lsptr(as, PPCI_LFS, (fbias & 31), (void*)lj_ir_k64_find(as->J, U64x(59800004, 59800000)), RSET_GPR);
...
static void asm_conv(ASMState* as, IRIns* ir)
{
	...
	if (irt_isfp(ir->t))
	{
		...
		{ /* Integer to FP conversion. */
			...
			if (st == IRT_U32)
				kbias++;
			emit_lsptr(as, PPCI_LFS, (fbias & 31), (void*)kbias, rset_clear(allow, hibias));
	...
	{ /* FP to integer conversion. */
		...
		else
		{
			...
			if (irt_isu32(ir->t))
			{
				...
				emit_lsptr(as, PPCI_LFS, (tmp & 31), (void*)lj_ir_k64_find(as->J, U64x(4f000000, 00000000)), RSET_GPR);
			}
			else
			{
				emit_tai(as, PPCI_LWZ, dest, RID_SP, SPOFS_TMPLO);
...
static void asm_xstore_(ASMState* as, IRIns* ir, int32_t ofs)
{
	...
	if (ir->r == RID_SINK)
		return;
	if (ofs == 0 && mayfuse(as, ir->op2) && (irb = IR(ir->op2))->o == IR_BSWAP && ra_noreg(irb->r) &&
...
static void asm_bswap(ASMState* as, IRIns* ir)
{
	Reg dest = ra_dest(as, ir, RSET_GPR);
	IRIns* irx;
	if (mayfuse(as, ir->op1) && (irx = IR(ir->op1))->o == IR_XLOAD && ra_noreg(irx->r) &&
...
static void asm_fuseandsh(ASMState* as, PPCIns pi, int32_t mask, IRRef ref)
{
	IRIns* ir;
	Reg left;
	if (mayfuse(as, ref) && (ir = IR(ref), ra_noreg(ir->r)) && irref_isk(ir->op2) && ir->o >= IR_BSHL &&
		ir->o <= IR_BROR)
	{
...
static void asm_band(ASMState* as, IRIns* ir)
{
	...
	if (irref_isk(ir->op2))
	{
		...
		if (k)
		{
			...
			{
				asm_fuseandsh(as,
							  PPCI_RLWINM | dot | PPCF_A(dest) | PPCF_MB(31 - lj_fls((uint32_t)k)) | PPCF_ME(31 - s1),
							  k,
							  lref);
				return;
			}
			if (~(uint32_t)k)
			{
				...
				{
					asm_fuseandsh(as,
								  PPCI_RLWINM | dot | PPCF_A(dest) | PPCF_MB(32 - s2) |
...
#define asm_brol(as, ir)                                                                                               \
	asm_bitshift(as, ir, PPCI_RLWNM | PPCF_MB(0) | PPCF_ME(31), PPCI_RLWINM | PPCF_MB(0) | PPCF_ME(31))
...
static void asm_intcomp_(ASMState* as, IRRef lref, IRRef rref, Reg cr, PPCCC cc)
{
	...
	if (irref_isk(rref))
	{
		int32_t k = IR(rref)->i;
		if ((cc & CC_UNSIGNED) == 0)
		{ /* Signed comparison with constant. */
			if (checki16(k))
			{
		...
		{ /* Unsigned comparison with constant. */
			if (checku16(k))
			{
				emit_tai(as, PPCI_CMPLWI, cr, left, k);
				return;
