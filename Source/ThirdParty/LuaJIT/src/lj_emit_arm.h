...
static void emit_lso(ASMState* as, ARMIns ai, Reg rd, Reg rn, int32_t ofs)
{
	...
	if (*as->mcp == (ai | ARMI_LS_P | ARMI_LS_U | ARMF_D(rd ^ 1) | ARMF_N(rn) | (ofs ^ 4)) &&
		(ai & ~(ARMI_LDR ^ ARMI_STR)) == ARMI_STR && rd != rn && (uint32_t)ofs <= 252 && !(ofs & 3) &&
...
static void emit_loadn(ASMState* as, Reg r, cTValue* tv)
{
	...
	if ((as->flags & JIT_F_VFPV3) && !tv->u32.lo)
	{
		...
		{
			*--as->mcp = ARMI_VMOVI_D | ARMF_D(r & 15) | ((tv->u32.hi >> 12) & 0x00080000) |
						 ((tv->u32.hi >> 4) & 0x00070000) | ((tv->u32.hi >> 16) & 0x0000000f);
			return;
		}
	}
	i = i32ptr(tv);
