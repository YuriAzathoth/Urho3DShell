...
#define emit_opm(xo, mode, rr, rb, p, delta)                                                                           \
	(p[(delta)-1] = MODRM((mode), (rr), (rb)), emit_op((xo), (rr), (rb), 0, (p), (delta)))

/* op + modrm + sib */
#define emit_opmx(xo, mode, scale, rr, rb, rx, p)                                                                      \
	(p[-1] = MODRM((scale), (rx), (rb)), p[-2] = MODRM((mode), (rr), RID_ESP), emit_op((xo), (rr), (rb), (rx), (p), -1))
...
static void emit_loadi(ASMState* as, Reg r, int32_t i)
{
	...
	if (i == 0 && !(LJ_32 && (IR(as->curins)->o == IR_HIOP ||
							  (as->curins + 1 < as->T->nins && IR(as->curins + 1)->o == IR_HIOP))))
	{
