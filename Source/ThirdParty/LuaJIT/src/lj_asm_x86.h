...
static IRRef asm_fuseabase(ASMState* as, IRRef ref)
{
	...
	if (irb->o == IR_FLOAD)
	{
		...
		if (ira->o == IR_TNEW && ira->op1 <= LJ_MAX_COLOSIZE && !neverfuse(as) &&
...
static Reg asm_fuseload(ASMState* as, IRRef ref, RegSet allow)
{
	...
	if (ir->o == IR_KNUM)
	{
		...
		if (!(avail & (avail - 1)))
		{ /* Fuse if less than two regs available. */
	...
	else if (mayfuse(as, ref))
	{
		...
		if (ir->o == IR_SLOAD)
		{
			if (!(ir->op2 & (IRSLOAD_PARENT | IRSLOAD_CONVERT)) && noconflict(as, ref, IR_RETF, 0))
			{
	...
	if (!(as->freeset & allow) && !irref_isk(ref) && (allow == RSET_EMPTY || ra_hasspill(ir->s) || iscrossref(as, ref)))
		goto fusespill;
...
static void asm_setupresult(ASMState* as, IRIns* ir, const CCallInfo* ci)
{
	...
	if (ra_used(ir))
	{
		if (irt_isfp(ir->t))
		{
			...
			{
				emit_rmro(as,
						  irt_isnum(ir->t) ? XO_FSTPq : XO_FSTPd,
...
static void asm_conv(ASMState* as, IRIns* ir)
{
	...
	if (irt_isfp(ir->t))
	{
		...
		{ /* Integer to FP conversion. */
			Reg left = (LJ_64 && (st == IRT_U32 || st == IRT_U64)) ? ra_alloc1(as, lref, RSET_GPR)
			...
			emit_mrm(as,
					 irt_isnum(ir->t) ? XO_CVTSI2SD : XO_CVTSI2SS,
	...
	{ /* FP to integer conversion. */
		...
		else
		{
			...
			if (LJ_64 ? irt_isu64(ir->t) : irt_isu32(ir->t))
			{
				...
				if (st == IRT_NUM)
					emit_rma(as,
							 XO_ADDSD,
							 tmp,
							 lj_ir_k64_find(as->J, LJ_64 ? U64x(c3f00000, 00000000) : U64x(c1e00000, 00000000)));
				else
					emit_rma(as,
							 XO_ADDSS,
							 tmp,
							 lj_ir_k64_find(as->J, LJ_64 ? U64x(00000000, df800000) : U64x(00000000, cf000000)));
				emit_sjcc(as, CC_NS, l_end);
			...
			else
			{
				...
				emit_mrm(as, op, dest | ((LJ_64 && (irt_is64(ir->t) || irt_isu32(ir->t))) ? REX_64 : 0), left);
			}
		}
	}
...
static void asm_conv_fp_int64(ASMState* as, IRIns* ir)
{
	...
	if (((ir - 1)->op2 & IRCONV_SRCMASK) == IRT_U64)
	{
		...
		MCLabel l_end = emit_label(as);
		emit_rma(as, XO_FADDq, XOg_FADDq, lj_ir_k64_find(as->J, U64x(43f00000, 00000000)));
		emit_sjcc(as, CC_NS, l_end);
...
static void asm_conv_int64_fp(ASMState* as, IRIns* ir)
{
	...
	if (dt == IRT_U64)
	{
		...
		emit_rma(as, XO_FADDq, XOg_FADDq, lj_ir_k64_find(as->J, U64x(c3f00000, 00000000)));
	...
	{ /* Otherwise set FPU rounding mode to truncate before the store. */
		emit_rmro(as, XO_FISTPq, XOg_FISTPq, RID_ESP, 0);
...
static void asm_href(ASMState* as, IRIns* ir, IROp merge)
{
	...
	else
	{
		...
		{ /* Must match with hashrot() in lj_tab.c. */
...
static void asm_uref(ASMState* as, IRIns* ir)
{
	...
	else
	{
		...
		emit_rmro(as, XO_MOV, uv, func, (int32_t)offsetof(GCfuncL, uvptr) + 4 * (int32_t)(ir->op2 >> 8));
	}
}
...
static void asm_x87load(ASMState* as, IRRef ref)
{
	...
	else if (ir->o == IR_CONV && ir->op2 == IRCONV_NUM_INT && !ra_used(ir) && !irref_isk(ir->op1) &&
			 mayfuse(as, ir->op1))
	{
		IRIns* iri = IR(ir->op1);
...
static void asm_fpmath(ASMState* as, IRIns* ir)
{
	...
	else if (fpm <= IRFPM_TRUNC)
	{
		...
		{ /* Call helper functions for SSE2 variant. */
...
static int asm_lea(ASMState* as, IRIns* ir)
{
	...
	if (ra_hasreg(irl->r))
	{
		...
		if (irref_isk(ir->op2) || ra_hasreg(irr->r))
		{
			...
			if (ra_hasreg(ir->r) && ((irt_isphi(irl->t) && as->phireg[ir->r] == ir->op1) ||
...
static void asm_bitshift(ASMState* as, IRIns* ir, x86Shift xs)
{
	...
	{ /* Variable shifts implicitly use register cl (i.e. ecx). */
		Reg right;
	...
	else
	{
		...
		if (asm_isk32(as, rref, &imm))
		{
			...
			if (usetest && irl->o == IR_BAND && irl + 1 == ir && !ra_used(irl))
			{
				...
				{ /* Try to Fuse IRT_I8/IRT_U8 loads, too. See below. */
			...
			else
			{
				...
				if (opisfusableload((IROp)irl->o) &&
					((irt_isu8(irl->t) && checku8(imm)) || ((irt_isi8(irl->t) || irt_isi16(irl->t)) && checki8(imm)) ||
					 (irt_isu16(irl->t) && checku16(imm) && checki8((int16_t)imm))))
				{
					...
					*/
					...
					{ /* Fusion succeeded? */
						...
						emit_i8(as, imm);
						emit_mrm(as,
								 (irt_isi8(origt) || irt_isu8(origt)) ? XO_ARITHib : XO_ARITHiw8,
								 r64 + XOg_CMP,
								 RID_MRM);
						return;
...
static void asm_setup_target(ASMState* as) { asm_exitstub_setup(as, as->T->nsnap); }
