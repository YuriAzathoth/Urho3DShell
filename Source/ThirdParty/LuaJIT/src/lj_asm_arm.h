...
static void asm_fusexref(ASMState* as, ARMIns ai, Reg rd, IRRef ref, RegSet allow, int32_t ofs)
{
	...
	if (ra_noreg(ir->r) && canfuse(as, ir))
	{
		...
		{
			int32_t ofs2;
			if (irref_isk(ir->op2) && (ofs2 = ofs + IR(ir->op2)->i) > -lim && ofs2 < lim &&
...
static int asm_fusemadd(ASMState* as, IRIns* ir, ARMIns ai, ARMIns air)
{
	...
	if (lref != rref &&
		((mayfuse(as, lref) && (irm = IR(lref), irm->o == IR_MUL) && ra_noreg(irm->r)) ||
		 (mayfuse(as, rref) && (irm = IR(rref), irm->o == IR_MUL) && (rref = lref, ai = air, ra_noreg(irm->r)))))
	{
...
static void asm_conv(ASMState* as, IRIns* ir)
{
	...
	if (irt_isfp(ir->t))
	{
		...
		{ /* Integer to FP conversion. */
			...
			ARMIns ai = irt_isfloat(ir->t) ? (st == IRT_INT ? ARMI_VCVT_F32_S32 : ARMI_VCVT_F32_U32)
	...
	{ /* FP to integer conversion. */
		...
		else
		{
			...
			ARMIns ai;
			emit_dn(as, ARMI_VMOV_R_S, dest, (tmp & 15));
			ai = irt_isint(ir->t) ? (st == IRT_NUM ? ARMI_VCVT_S32_F64 : ARMI_VCVT_S32_F32)
...
static void asm_strref(ASMState* as, IRIns* ir)
{
	...
	else if (!irref_isk(refk))
	{
		...
		else if (mayfuse(as, irr->op2) && irr->o == IR_ADD && irref_isk(irr->op2) &&
				 (k = emit_isk12(ARMI_ADD, (int32_t)sizeof(GCstr) + IR(irr->op2)->i)))
		{
			m = k;
